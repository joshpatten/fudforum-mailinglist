...
function getFromAddr($emailarr) {
	foreach ($emailarr as $mailline) {
        if (preg_match('/^X-Original-To: +/', $mailline)) {
                $mailline = str_replace('X-Original-To: ', '', $mailline);
                $origaddr = trim($mailline);
                return $origaddr;
        }
	}
}
...
//Set proper X-Mailer header
errorReport("Detecting duplicate X-Mailer headers\n");
$xmailer = getXMailer($email);
...
$subject_prepend = str_replace('/', "", $subject_prepend);
errorReport("Mailing list subject line injection is: " . $subject_prepend . "\n");
...
//For some reason first entry in email array from FUDforum is blank...
errorReport("Fixing null first line\n");
unset($email[0]);
...
foreach ($email as $mailline) {
	if (preg_match('/^From: +/', $mailline)) {
    errorReport("Ignoring line " . $mailline);
	} else if (preg_match('/^Errors-To: +/', $mailline)) {
    ...
    $mailline = explode(" ", $mailline);
    $maillinestr = $mailline[0] . " " . $forumname . " <" . trim($mailline[1]) . ">\n";
    errorReport("New To: header is: " . trim($maillinestr) . "\n");
    $newemail[] = $maillinestr;
    //Add X-Mailer header
    $newemail[] = $xmailer;
  } else if (preg_match('/^X-Mailer: +/', $mailline)) {
    errorReport("Ignoring duplicate X-Mailer: header.\n");
	} else if (preg_match('/^Subject: +/', $mailline)) {
    if (preg_match('/^Subject: Re:+/', $mailline)) {
      $mailline = preg_replace('/^Subject: Re:+/', 'Subject: Re: ' . $subject_prepend, $mailline);
      $newemail[] = $mailline;
    } else {
      $mailline = preg_replace('/^Subject: +/', 'Subject: ' . $subject_prepend . ' ', $mailline);
      $newemail[] = $mailline;
    }
	} else {
		$newemail[] = $mailline;
